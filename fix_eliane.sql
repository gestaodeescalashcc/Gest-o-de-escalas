-- =============================================================================
-- Reativa a Eliane Reis dos Santos, atualiza pro setor correto
-- e insere seus 13 turnos de maio/2026
-- =============================================================================

-- 1) Reativa a Eliane do setor "Unidade de Internação" (vamos manter essa, FESFSUS)
--    e move pro setor "Técnicos de Enfermagem"
UPDATE professionals
SET active = true,
    department_id = (SELECT id FROM departments WHERE name = 'Técnicos de Enfermagem' LIMIT 1),
    company_id = (SELECT id FROM companies WHERE name ILIKE 'FESF%' LIMIT 1),
    updated_at = now()
WHERE id = '035c0714-867a-4b48-84cc-78da17e95378';

-- 2) Desativa a outra Eliane duplicada (a da Alfa, sem setor)
UPDATE professionals
SET active = false, updated_at = now()
WHERE id = '36476609-c5ee-4d63-b175-38e1ca91747a';

-- 3) Insere os 13 turnos
...
SELECT
  '035c0714-867a-4b48-84cc-78da17e95378'::uuid,
  ms.id,
  d.id,
...
SELECT d.name AS setor, COUNT(*) AS qtd
FROM shifts s
JOIN monthly_schedules ms ON ms.id = s.schedule_id
