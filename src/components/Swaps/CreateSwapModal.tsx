...
export default function CreateSwapModal({ onClose, onSuccess, initialShiftId }: CreateSwapModalProps) {
  ...
  const getAvailableShiftsForOffer = () => {
    ...
    return shifts.filter(s =>
      s.professional.id === requestingProfId &&
              ...
              {step === 3 && (
                <div className="space-y-4">
                  <p className="text-sm text-gray-600 bg-blue-50 border border-blue-200 rounded-lg p-3">
                    Oferecendo um plantão em troca, você aumenta as chances de aprovação!
                  </p>
                  ...
                  <div className="space-y-2 max-h-96 overflow-y-auto">
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-semibold text-gray-700">Não oferecer plantão</span>
                        {!formData.offered_shift_id && <CheckCircle className="w-6 h-6 text-gray-600" />}
                      </div>
                    </button>
                    ...
                    {getAvailableShiftsForOffer().length === 0 && (
                      <div className="text-center py-8 text-gray-500">
                        <p>Você não tem outros plantões disponíveis para oferecer</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {offeredShift && (
                  <>
                    ...
                    <div className="bg-white rounded-lg p-3 border border-green-200">
                      <p className="text-xs text-green-600 font-semibold mb-2">OFERECENDO</p>
                      <div className="text-xs text-gray-700 space-y-1">
                        <div className="flex items-center gap-2">
                          <Calendar className="w-3 h-3" />
